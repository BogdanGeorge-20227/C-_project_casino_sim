...
*/
   ...
   //     ♠ is char 6
    ...
    std::string PlayersChoice(std::string correct1,std::string correct2,std::string correct3,std::string correct4){
        std::string input;
        while(true){
            std::cin>>input;
...
class Customer:public Person{
    ...
    compstr="client "+std::to_string(id)+" dialogue "+std::to_string(addiction_level+1)+':'; //header formation ex: "John bond 2:"
    //std::cout<<compstr<<std::endl;
    //if(!ReadCuststats.is_open())std::cout<<"Stinkyyyy"<<std::endl;
...
class CardDeck{
    ...
    CardDeck(bool Rigged){
        if(Rigged){
            ...
            base_card=ace_under_the_sleeve;
        }
    }
    //some getcard function that also puts the returned card at the bottom of the draw pile
    card GetCard(){
    ...
    void ShuffleDeck(int seed){
        ...
        for(int shufflecount=0;shufflecount<=seed;shufflecount++){
            ...
            while(i_midend<deck.size()-1){
                 ...
                 deck.erase(deck.begin()+i_midend);        //by adjusting the number of times the deck is shuffled we create a seeding system, since shuffling is deterministic
...
void SimulateBlackJack(int bet, CardDeck* play_deck,bool RiggedCustomer,bool RiggedDealer,int seed,bool &dealerwin,bool &draw){ 
    play_deck->ShuffleDeck(seed);
    dealerwin=false;
    draw=false;
    int cust_sum=0;
    ...
    while(cust_turn){
        ...
        if(RiggedCustomer&&cust_sum==10)customer_hand->DrawEvaluation(first_turn,cust_blackjack,cust_turn,false,0,cust_sum,customer_hand->AceUnderSleeve());
    }
...
class Casino{
    ...
    std::vector<std::shared_ptr<Customer>> customers_present,all_customers;
    std::vector<Employee*> employees_present,all_employees;
    std::vector<std::string> news;
    std::map<std::string, int> crimesCommitted;
    std::map<std::string, bool> bonded_today;
    CardDeck* play_deck;
    enum DailyRoutine:unsigned char
    {
        Morning=0,
        News,
        Tables,
    ...
    void CrimeInit(){
        ...
        crimesCommitted.insert({"Public intoxication",0});
    ...
    void NewsCycleInit(){
    std::ifstream ReadNews("News_stories.txt");   //open up dialogue file !!!FILE NEEDS TO BE NEXT TO .EXE FILE!!! (for me that was in "output" folder)
    ...
    std::ifstream ReadCustStats("Customer_stats.txt");   //open up stats file !!!FILE NEEDS TO BE NEXT TO .EXE FILE!!! (for me that was in "output" folder)
    ...
    while(std::getline(ReadCustStats,currentstr)){ 
        ...
        if(currentstr==compstr){                               
                ...
                std::getline(ReadCustStats,currentstr); 
                if(currentstr!="@%") all_customers.push_back(std::shared_ptr<Customer>(new MobsterCustomer(i,money,salary,read_cust_name,addiction_lvl,std::stoi(currentstr))));
                else all_customers.push_back(std::shared_ptr<Customer>(new Customer(i,money,salary,read_cust_name,addiction_lvl)));  //money salary name addiction  
                //all_customers.at(i-1)->DebugDisplayAllInfo();       
                i++;                                  
         }
    }
    //std::cout<<all_customers.size();
    ReadCustStats.close();      //closing the file
    }
    void AllEmployeesInit(){
        std::ifstream ReadEmployeeStats("Employee_stats.txt");   //open up stats file !!!FILE NEEDS TO BE NEXT TO .EXE FILE!!! (for me that was in "output" folder)
    ...
    while(std::getline(ReadEmployeeStats,currentstr)){ 
        ...
        if(currentstr==compstr){                               
                ...
                std::getline(ReadEmployeeStats,currentstr); 
                addiction_lvl=std::stoi(currentstr);    
                all_employees.push_back(new Employee(i,money,salary,read_employee_name,addiction_lvl));  //money salary name profits  
                //all_employees.at(i-1)->DebugDisplayAllInfo();       
                i++;                                  
         }
    }
    ...
    void CustomerArrival(int current_day){
        ...
        for(int i=0;i<=2;i++){
            ...
            customers_present.at(i)->DialogueInit();
        }
    }
    ...
    void UpdateCrimeMeter(){
        ...
        for(auto const& [key, val]:crimesCommitted){
            ...
            if(key=="Conspiracy to international terrorism")crime_meter=crime_meter+val*450;
            else{
    ...
    while(!exit){
        switch(time_of_day){
            ...
            case Play:
            {
             ...
             if(bet>customers_present.at(table_choice)->getMoney()/5){
             ...
             SimulateBlackJack(bet,play_deck,false, false,seed,game_dealer_win,game_draw);
             if(game_draw){}
                else{
                if(game_dealer_win){
                    UpdateProfits(bet);
            ...
            case Bond:
            {
                if(!bonded_today.at(customers_present.at(table_choice)->getName())){
            ...
            case NightBreakdown:
            {
                std::cout<<std::endl<<"Good job, today you made "<<profits<<" money, how much do you think you should pay in taxes?[Input a number]";
                std::cin>>tax;
                if(tax==0){
                    //commit crime tmr lol
                }
                else{
                    if(tax<profits*tax_rate/100);//commit low tax evasion
                }
                std::cout<<std::endl<<"After tax you're left with a total of "<<funds<<" money"<<std::endl;
                UpdateFunds(profits-tax);
                std::cout<<"You could also employ someone or give your mob friends a call and see if they got anything for you. [Employ]/[Do some crime]/[End the day]"<<std::endl;
                input=PlayersChoice("Employ","Do some crime","End the day");
                if(input=="Employ")time_of_day=Employ;
                if(input=="Do some crime")time_of_day=Crime;
                if(input=="End the day")time_of_day=EndCheck;
                profits=0;
            break;
            }
            case Crime:
            {
            std::cout<<"You may help the mob in it's activities, yielding lots of money, or just have some fun crimes as a treat [Help mob]/[jaywalk]/[do some coke]/[drink until you pass out]"<<std::endl;
            input=PlayersChoice("Help mob","jaywalk","do some coke","drink until you pass out");
            if(input=="Help mob")CommitCrime("Conspiracy to international terrorism");
            if(input=="jaywalk")CommitCrime("Jaywalking");
            if(input=="do some coke")CommitCrime("Illegal substance abuse");
            if(input=="drink until you pass out")CommitCrime("Public intoxication");
            ...
            case EndCheck:
            {
                if(crime_meter>1000){
                   ...
                   if(crime_meter>900 && warning_level==-1)warning_level=2;
                         ...
                         if(crime_meter<300)warning_level=0;
                        }
                    if(funds>2000000){
